...
export default function PersonDetail({ personId, setPersonId }) {
    ...
    const fetchPerson = async () => {
        try {
            const response = await fetch(`api/people/${personId}`); // Use template literals to interpolate personId
    ...
    useEffect(() => {
        fetchPerson();
    }, [personId]); // Fetch data whenever personId changes
            ...
            </div>
            {
                person ? (
                    <div className="people-list__person-details">
                        ...
                        <h2>Looks:</h2>
                        <div>Hair color: {person.hair_color ?? 'Unknown'}</div><br />
                        <div>Eye color: {person.eye_color ?? 'Unknown'}</div><br />
